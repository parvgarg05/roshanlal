...
/**
 * Prisma-safe type that matches the query used in notifications.ts
 */
...
const FROM_EMAIL =
  getEnv('FROM_EMAIL') || 'info@roshanlaljisweets.com';

const ADMIN_EMAIL =
  getEnv('ADMIN_EMAIL') || 'info@roshanlaljisweets.com';
...
export async function sendCustomerOrderEmail(
  ...
  if (!isConfigured()) {
    console.warn(
      '⚠️ [Mail] No SMTP config found. Skipping customer email for order:',
      order.id
    );
    return false;
  }
  ...
  if (!customer) {
    console.warn('⚠️ [Mail] No customer found for order:', order.id);
    return false;
  }
  ...
  const html = `
    <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; color: #431407;">
      <div style="background: linear-gradient(135deg, #f97316 0%, #db2777 100%);
                  padding: 30px; text-align: center; border-radius: 12px 12px 0 0;">
        <h1 style="color: white; margin: 0;">Order Confirmed!</h1>
      </div>

      <div style="padding: 30px; background-color: #fffaf0;
                  border: 1px solid #fcebb6; border-top: none;
                  border-radius: 0 0 12px 12px;">

        <p>Dear <strong>${customer.name}</strong>,</p>
        <p>
          Thank you for ordering from L. Roshanlal Ji Sweets.
          We are preparing your fresh mithai now!
        </p>
        ...
        <div style="background-color: white; border-radius: 8px;
          ...
          <h3 style="margin-top: 0; color: #9f1239;
            ...
            ${
              ...
              <tr>
                ...
                <td style="padding: 10px; text-align: right; color: #666;">
                  ${formatCurrency(order.cgstTotalPaise / 100)}
                </td>
              </tr>
              <tr>
                <td style="padding: 10px; text-align: right; color: #666;">SGST</td>
                <td style="padding: 10px; text-align: right; color: #666;">
                  ${formatCurrency(order.sgstTotalPaise / 100)}
                </td>
              </tr>
            `
                : ''
              ...
              <td style="padding: 10px; text-align: right;">
                ${
                  order.deliveryPaise === 0
                    ? 'FREE'
                    : formatCurrency(order.deliveryPaise / 100)
                }
              </td>
            </tr>

            <tr>
              <td style="padding: 10px; text-align: right; font-size: 1.1em; color: #9f1239;">
                <strong>Grand Total</strong>
              </td>
              <td style="padding: 10px; text-align: right; font-size: 1.1em; color: #9f1239;">
                <strong>${formatCurrency(order.totalPaise / 100)}</strong>
              </td>
            </tr>
          </table>
        </div>
  ...
  try {
    ...
    console.log('✅ [Mail] Confirmation sent:', info.messageId);
    return true;
  } catch (error) {
...
export async function sendAdminAlertEmail(
  ...
  if (!customer) {
    console.warn('⚠️ [Mail] No customer for admin alert:', order.id);
    return false;
  }
  ...
  const html = `
    ...
    <p><strong>Amount:</strong> ₹${order.totalPaise / 100}</p>
    <hr/>
  ...
  try {
    const info = await transporter.sendMail({
      from: `"L. Roshanlal Ji Sweets" <${FROM_EMAIL}>`,
      to: ADMIN_EMAIL,
      subject: `🚨 New Order - ₹${order.totalPaise / 100}`,
      html,
    });
